			...
			<div class="row">
				<div class="col-12">
					<div class="bread-inner">
						<ul class="bread-list">
							<li><a href="{{('home')}}">{{__('product.Home')}}<i class="ti-arrow-right"></i></a></li>
							<li class="active"><a href="">{{__('product.Cart')}}</a></li>
						</ul>
					</div>
				</div>
			</div>
					...
					<table class="table shopping-summery">
						<thead>
							<tr class="main-hading">
								...
								<th class="text-center">{{__('product.UNIT PRICE')}}</th>
								<th class="text-center">{{__('product.QUANTITY')}}</th>
								<th class="text-center">{{__('product.TOTAL')}}</th>
								...
								@if(Helper::getAllProductFromCart())
									...
									<track>
										...
										<td class="float-right">
											<button class="btn float-right" type="submit">{{__('product.Update')}}</button>
										</td>
									</track>
								@else
										<tr>
							...
							<div class="col-lg-8 col-md-5 col-12">
								<div class="left">
									...
									<form action="{{route('coupon-store')}}" method="POST">
											@csrf
											<input name="code" placeholder="{{__('product.Enter Your Coupon')}}">
											<button class="btn">{{__('product.Apply')}}</button>
										</form>
									</div>
									 <div class="checkbox">`
										@php
							...
							<div class="col-lg-4 col-md-7 col-12">
								<div class="right">
									<ul>
										...
										<li class="coupon_price" data-price="{{Session::get('coupon')['value']}}">{{__('product.You Save')}}<span>€{{number_format(Session::get('coupon')['value'],2)}}</span></li>
										@endif
										@php
										...
										@endphp
										@if(session()->has('coupon'))
											<li class="last" id="order_total_price">{{__('product.You Pay')}}<span>€{{number_format($total_amount,2)}}</span></li>
										@else
											<li class="last" id="order_total_price">{{__('product.You Pay')}}<span>€{{number_format($total_amount,2)}}</span></li>
										@endif
									</ul>
									<div class="button5">
										<a href="{{route('checkout')}}" class="btn">{{__('product.Checkout')}}</a>
										<a href="{{route('product-grids')}}" class="btn">{{__('product.Continue shopping')}}</a>
									</div>
								</div>
							</div>
						</div>
